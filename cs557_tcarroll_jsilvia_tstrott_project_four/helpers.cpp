...
bool check_for_peer(string peer, string peerlist_loc) {
    ...
    if (!check_for_datastore(peerlist_loc.c_str())) {
        // File does not exist
        return false;
    }

    infile.open(peerlist_loc.c_str(), fstream::in | fstream::app);
    ...
    while(getline(infile, line)) {
        ...
        if (user.compare(peer) == 0) {
            // peer exists in peerlist
            infile.close();
            return true;
        }
    }
...
int get_port_for_peer(string peer, string peerlist_loc) {
    ...
    while(getline(infile, line)) {
        user = line.substr(0,line.find(","));
        if (user.compare(peer) == 0) {
            // get port for this peer
            stringstream stream(line);
            string port_string;
            //get peer
            getline(stream, port_string, ',');
            //get port
            getline(stream, port_string, ',');
            infile.close();
...
bool add_peer(string peer, int port, string peerlist_loc) {
    ...
    outfile.open(peerlist_loc.c_str(), fstream::out | fstream::app);

    if (outfile.fail()) {
        return false;
    }

    outfile << peer << "," << port << endl;
